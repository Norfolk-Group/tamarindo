...
- Tailwind + shadcn/ui
- Prisma + PostgreSQL
- WorkOS AuthKit (see `docs/nico/02-tech-stack.md`)
...
- Verify `WORKOS_API_KEY`, `WORKOS_CLIENT_ID`, and `WORKOS_COOKIE_PASSWORD` are set
...
**Auth issues**
- Verify WorkOS keys in `.env` match the Tamarindo Staging app at dashboard.workos.com
- Confirm `/login` and `/callback` match the AuthKit initiate-login and redirect URIs
