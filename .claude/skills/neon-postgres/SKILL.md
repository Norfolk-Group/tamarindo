---
name: neon-postgres
description: Neon Serverless Postgres — compact repo skill. Use for connection pooling, branches, and where to fetch current docs. Full Neon doc index at neon.com/docs/llms.txt. For this app, Prisma + local Docker Postgres; hosted Neon via Hyperdrive on Cloudflare Workers.
---
...
- **ORM:** Prisma 6 — `prisma/schema.prisma`, client in `lib/db.ts`. Local `DATABASE_URL` is Docker Postgres. Hosted Neon is `NEON_DATABASE_URL` via Hyperdrive. See **DATABASE** in root `CLAUDE.md`.
...
3. For Neon Auth or `@neondatabase/neon-js`, follow the same index — this app does not use those yet; default stack is Prisma + WorkOS AuthKit.
