...
- Financial statements, investor returns, sensitivity, and the corporate-structure diagram arrive as a glance on screen. Summary first; Extended is every line. Returns and sensitivity may add a comparison chart. Do not reprint the fence. Do not invent an IRR, an exit year, or a raise. If the engine left a return blank, say so. The full book (or the structure diagram) opens in a new tab. PDF, CSV, and Excel download from that glance. A full family workbook also lands in Files if they asked for a worksheet.
